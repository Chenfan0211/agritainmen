...
- `apps/supplier`：供应商配送工作台，供应商与司机双角色登录；供应商处理门店进货单的接单/派单/快递直发/出库交接并管理司机账号，司机查看当日配送任务并完成到店交接，缺货自动统计。仅构建 H5。
...
预览端口：admin 8791、farmhouse 8792、alliance 8793、云上人家 8794、store 8795、promoter 8796、user 8797、supplier 8798（见 `scripts/serve-dist.mjs`）。跨应用联动（直播发布、用户绑定、消费分成、门店账号）依赖 H5 同域名部署；开发时不同端口不共享，可用构建产物挂同源路径验证。
