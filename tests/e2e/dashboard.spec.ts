...
test('authenticates the leader and renders authorized regional comparison', async ({ page }) => {
  ...
  await expect(page.locator('.map-chart canvas')).toBeVisible()
...
test('loads city and district maps and returns through the authorized breadcrumb', async ({ page }) => {
  ...
  await expect(page.locator('#farm-map-chart canvas')).toBeVisible()
  await expect(page.locator('#farm-map-chart')).toHaveAttribute('aria-label', /长沙市门店经营分布.*橘子洲畔农家院.*长沙捞刀河渔家乐.*长沙洋湖荷塘月色院/)
