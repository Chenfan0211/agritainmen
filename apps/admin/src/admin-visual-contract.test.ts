...
describe('admin visual contract', () => {
  ...
  it('uses token sidebar width and a clean login without farm photography', () => {
    ...
    expect(pageSource).toMatch(/\.nav-item\.active\s*\{[^}]*box-shadow:inset 3px 0 0/s)
    expect(pageSource).toMatch(/\.row-actions button:first-child:not\(\.danger\)\{background:#1d6b44/s)
    ...
    expect(pageSource).toMatch(/\.product-tag\{[^}]*border-radius:4px/)
    ...
    expect(pageSource).toMatch(/\.switch \{[^}]*border-radius:8px/)
    ...
    expect(pageSource).toMatch(/\.compact-button\.danger \{[^}]*background: #f8e9e6/)
  ...
  it('宽表只在内层横滑，供应商操作列可换行且筛选走 token', () => {
    ...
    expect(pageSource).toMatch(/\.report-filters input\{height:var\(--admin-control-height\)/)
