...
import { createId, formatNumber, installKeyboardButtonSupport, money, validatePhone } from '@agritainment/shared'
  ...
  <view class="app-shell">
    ...
    <template v-else>
      ...
      <view v-else-if="activeTab === 'rank'" class="tab-page">
        ...
        <view class="rank-list"><button v-for="(farm,index) in rankedFarms" :key="farm.id" @click="openFarm(farm)"><text class="rank-number" :class="`rank-medal-${index < 3 ? index + 1 : 'n'}`">{{ index + 1 }}</text><image class="rank-emoji" :src="farm.image" mode="aspectFill" /><view><text>{{ farm.name }}</text><small>{{ rankRegion(farm) }} · {{ rankType === '好评榜' ? `评分 ${farm.rating}` : `月售 ${farm.monthlySales}` }}</small></view><view class="rank-value"><strong>{{ rankType === '好评榜' ? farm.rating : farm.livePopularity }}</strong><small>人气值</small></view></button></view>
        <view class="section-head"><view><span></span><text>乡村旅游线路推荐</text></view></view>
        <view class="route-list"><view v-for="route in store.routes" :key="route.id" class="route-card"><image class="route-emoji" :src="route.image" mode="aspectFill" /><view><text>{{ route.name }}</text><small>{{ route.description }}</small><small v-if="route.meta" class="route-meta">{{ route.meta }}</small><button @click="openRoute(route)">查看线路</button></view></view><view v-if="!store.cityRoutes.length" class="empty">当前城市暂无线路</view></view>
      </view>
...
<view class="live-note"><view><text>直播说明</text><small>🛒 主播可销售农家乐套餐、农产品、伴手礼及供应链商品。</small><small>🏬 直播成交后，由供应链中台统一承接发货、售后与结算，减少农家乐履约压力。</small><small>💰 成交订单自动追踪推客 / 主播佣金，按规则结算。</small></view></view>
      ...
      <view v-else class="tab-page promoter-page">
        ...
        <view class="promoter-head"><view class="profile"><view class="pav"><text>🧑‍💼</text></view><view><text class="pname">推客 · {{ store.promoter?.name }}</text><small class="plv">联盟推客<text class="vip">{{ store.promoter?.level }}</text></small></view></view></view><view class="wallet-card"><view class="wallet-top"><view><small>可提现佣金（元）</small><strong>{{ store.availableCommission.toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</strong></view><button @click="openWithdraw">提现</button></view><view class="wallet-stats"><view><text>{{ money(store.cumulativeCommission) }}</text><small>累计佣金</small></view><view><text>{{ store.promoter?.fans }}</text><small>锁定粉丝</small></view><view><text>{{ store.promoter?.orders }}</text><small>本月订单</small></view></view></view>
      ...
      <view v-if="selectedFarm" class="preview-mask" @click.self="selectedFarm = null"><view class="preview"><button class="preview-close" aria-label="关闭门店预览" @click="selectedFarm = null"><UiIcon name="x" :size="19" /></button><image :src="selectedFarm.image" mode="aspectFill" /><view class="preview-body"><text>{{ selectedFarm.name }}</text><small>{{ selectedFarm.region }} · 距你 {{ selectedFarm.distance }}km</small><view class="tag-row"><text v-for="tag in selectedFarm.tags" :key="tag">{{ tag }}</text></view><view class="preview-actions"><button :disabled="selectedFarm.availability !== 'bookable'" @click="openBooking"><UiIcon name="calendar-days" :size="17" />{{ selectedFarm.availability === 'bookable' ? '立即预订' : '暂不可订' }}</button><button @click="openFarmShop"><UiIcon name="shopping-bag" :size="17" />逛特产</button><button @click="shareFarm"><UiIcon name="link" :size="17" />分享</button></view><view class="preview-products"><view v-for="product in farmProducts(selectedFarm).slice(0, 2)" :key="product.id"><image :src="product.image" mode="aspectFill" /><text>{{ product.name }}</text><strong>{{ money(product.price) }}</strong></view><view v-if="!farmProducts(selectedFarm).length" class="empty">该门店暂无特产</view></view></view></view></view>
...
<view v-if="sheet === 'login'" class="login-sheet">
        ...
        <view v-else-if="sheet === 'live' && selectedLive" class="live-detail"><image :src="selectedLive.image" mode="aspectFill" /><view class="live-state"><span></span>前端模拟直播间</view><text>{{ selectedLive.title }}</text><small>{{ selectedLive.host }} · {{ selectedLive.viewers.toLocaleString('zh-CN') }} 人观看</small><view><strong>{{ selectedLive.productName }}</strong><b>{{ money(selectedLive.productPrice) }}</b></view><button class="primary-button" @click="shareLive">分享直播赚佣金</button></view>
        <view v-else-if="sheet === 'materials'" class="sheet-materials"><view v-if="!cityProducts.length" class="empty">当前城市暂无推广素材</view><view v-for="product in cityProducts" :key="product.id"><image :src="product.image" mode="aspectFill" /><view><text>{{ product.name }}</text><small>预计佣金 {{ money(commissionFor(product)) }}</small></view><button @click="openShare(product)">{{ store.sharedProductIds.includes(product.id) ? '再次分享' : '生成链接' }}</button></view></view>
        <view v-else-if="sheet === 'fans'" class="data-list"><view v-if="!store.fans.length" class="empty">暂无锁粉记录</view><view v-for="fan in store.fans" :key="fan.id"><view class="data-avatar"><UiIcon name="user-round" :size="18" /></view><view><text>{{ fan.name }}</text><small>{{ fan.source }} · {{ fan.lockedAt }}</small></view><span>已锁粉</span></view></view>
        ...
        <view v-else-if="sheet === 'farm-shop'" class="sheet-materials"><view v-if="!farmProducts(selectedShopFarm).length" class="empty">该门店暂无特产</view><view v-for="product in farmProducts(selectedShopFarm)" :key="product.id"><image :src="product.image" mode="aspectFill" /><view><text>{{ product.name }}</text><small>{{ product.tags.join(' · ') }} · {{ money(product.price) }}</small></view><button @click="openShare(product)">分享赚佣金</button></view></view>
...
.rank-tabs uni-button, .city-chips uni-button, .withdraw-chips uni-button, .filter-row uni-button { margin: 0; }
