...
import { PLATFORM_BINDINGS_STORAGE_KEY, PLATFORM_BOOKINGS_STORAGE_KEY, PLATFORM_CATALOG_STORAGE_KEY, PLATFORM_COMMISSION_LEDGER_STORAGE_KEY, PLATFORM_ENTITIES_STORAGE_KEY, PLATFORM_LIVES_STORAGE_KEY, PLATFORM_MEDIA_STORAGE_KEY, PLATFORM_PROMOTER_ACCOUNTS_STORAGE_KEY, PLATFORM_ROUTES_STORAGE_KEY, PLATFORM_SETTLEMENTS_STORAGE_KEY, PLATFORM_SHARE_CONFIG_STORAGE_KEY, PLATFORM_STORE_CATALOG_SELECTIONS_STORAGE_KEY, PLATFORM_WITHDRAWALS_STORAGE_KEY, createId, formatNumber, installKeyboardButtonSupport, money, readPlatformEntities, subscribePlatformChanges, validatePhone } from '@agritainment/shared'
...
const platformChangeKeys = [PLATFORM_ENTITIES_STORAGE_KEY, PLATFORM_PROMOTER_ACCOUNTS_STORAGE_KEY, PLATFORM_WITHDRAWALS_STORAGE_KEY, PLATFORM_BOOKINGS_STORAGE_KEY, PLATFORM_COMMISSION_LEDGER_STORAGE_KEY, PLATFORM_BINDINGS_STORAGE_KEY, PLATFORM_LIVES_STORAGE_KEY, PLATFORM_ROUTES_STORAGE_KEY, PLATFORM_MEDIA_STORAGE_KEY, PLATFORM_CATALOG_STORAGE_KEY, PLATFORM_STORE_CATALOG_SELECTIONS_STORAGE_KEY, PLATFORM_SHARE_CONFIG_STORAGE_KEY, PLATFORM_SETTLEMENTS_STORAGE_KEY]
const refreshSharedState = () => {
  if (store.auth.isLoggedIn) void store.refreshSharedState()
  else void store.initialize(true)
}
...
async function withdraw() {
  ...
  const result = await store.withdraw(Number(store.availableCommission.toFixed(2)), withdrawMethod.value, withdrawRequestKey.value)
  ...
  <view class="app-shell">
    ...
    <template v-else>
      ...
      <view v-else-if="activeTab === 'rank'" class="tab-page">
        ...
        <view class="rank-list"><button v-for="(farm,index) in rankedFarms" :key="farm.id" @click="openFarm(farm)"><text class="rank-number" :class="`rank-medal-${index < 3 ? index + 1 : 'n'}`">{{ index + 1 }}</text><BusinessImage class="rank-emoji" :src="farm.image" mode="aspectFit" /><view><text>{{ farm.name }}</text><small>{{ rankRegion(farm) }} · {{ rankType === '好评榜' ? `评分 ${farm.rating}` : `月售 ${farm.monthlySales}` }}</small></view><view class="rank-value"><strong>{{ rankType === '好评榜' ? farm.rating : farm.livePopularity }}</strong><small>人气值</small></view></button></view>
        <view class="section-head"><view><span></span><text>乡村旅游线路推荐</text></view></view>
        <view class="route-list"><view v-for="route in store.routes" :key="route.id" class="route-card"><BusinessImage class="route-emoji" :src="route.image" mode="aspectFit" /><view><text>{{ route.name }}</text><small>{{ route.description }}</small><small v-if="route.meta" class="route-meta">{{ route.meta }}</small><button @click="openRoute(route)">查看线路</button></view></view><view v-if="!store.cityRoutes.length" class="empty">当前城市暂无线路</view></view>
      </view>
      ...
      <view v-else-if="activeTab === 'live'" class="tab-page">
        ...
        <view class="live-list"><view v-for="(room,index) in store.liveRooms" :key="room.id" class="live-card" @click="room.status === 'live' ? watchLive(room) : (store.toggleReminder(room.id), toast(room.reminded ? '已预约开播提醒' : '已取消提醒'))"><view class="live-cover" :class="`live-cover-${(index % 4) + 1}`"><BusinessImage class="live-emoji" :src="room.image" mode="aspectFit" /><text :class="room.status">{{ room.status === 'live' ? '直播中' : '⏰ 19:30 预告' }}</text><small v-if="room.viewers">👁 {{ room.viewers >= 10000 ? (room.viewers / 10000).toFixed(1) + '万' : room.viewers }}</small></view><view class="live-body"><text>{{ room.title }}</text><small>🎙 {{ room.host }}{{ room.hostRole ? ' · ' + room.hostRole : '' }}</small><view v-if="room.status === 'live'"><span>🛒 在播：{{ room.productName }} {{ money(room.productPrice) }}</span></view><view v-else class="remind-row"><span>{{ room.reminded ? '✓ 已预约' : '🔔 预约提醒' }}</span></view></view></view></view>
<view class="live-note"><view><text>直播说明</text><small>🛒 主播可销售农家乐套餐、农产品、伴手礼及供应链商品。</small><small>🏬 直播成交后，由中选科技供应链中台统一承接发货、售后与结算，减少农家乐履约压力。</small><small>💰 成交订单自动追踪推客 / 主播佣金，按规则结算。</small></view></view>
      </view>
      ...
      <view v-else class="tab-page promoter-page">
        ...
        <view v-else class="promoter-login">
          <view class="promoter-login-card">
            ...
            <text class="promoter-login-title">登录中选科技推客中心</text>
      ...
      <view v-if="selectedFarm" class="preview-mask" @click.self="selectedFarm = null"><view class="preview"><button class="preview-close" aria-label="关闭门店预览" @click="selectedFarm = null"><UiIcon name="x" :size="19" /></button><BusinessImage :src="selectedFarm.image" mode="aspectFit" /><view class="preview-body"><text>{{ selectedFarm.name }}</text><small>{{ selectedFarm.region }} · 距你 {{ selectedFarm.distance }}km</small><view class="tag-row"><text v-for="tag in selectedFarm.tags" :key="tag">{{ tag }}</text></view><view class="preview-actions"><button :disabled="selectedFarm.availability !== 'bookable'" @click="openBooking"><UiIcon name="calendar-days" :size="17" />{{ selectedFarm.availability === 'bookable' ? '立即预订' : '暂不可订' }}</button><button @click="openFarmShop"><UiIcon name="shopping-bag" :size="17" />逛特产</button><button @click="shareFarm"><UiIcon name="link" :size="17" />分享</button></view><view class="preview-products"><view v-for="product in farmProducts(selectedFarm).slice(0, 2)" :key="product.id"><BusinessImage :src="product.image" mode="aspectFit" /><text>{{ product.name }}</text><strong>{{ money(product.price) }}</strong></view><view v-if="!farmProducts(selectedFarm).length" class="empty">该门店暂无特产</view></view></view></view></view>
...
<view v-if="sheet === 'login'" class="login-sheet">
        ...
        <view v-else-if="sheet === 'withdraw'" class="withdraw-form">
          <text class="withdraw-sub">可提现佣金 {{ '¥' + store.availableCommission.toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</text>
          <view class="withdraw-chips"><button v-for="item in ['微信','支付宝','银行卡']" :key="item" :class="{ sel: withdrawMethod === item }" @click="withdrawMethod = item">{{ item }}</button></view>
          <button class="primary-button" :disabled="withdrawing" @click="withdraw">{{ withdrawing ? '提交中...' : '确认提现' }}</button>
          <view class="withdrawal-history">
            <text class="withdrawal-history-title">提现记录</text>
            <view v-if="!store.withdrawalRecords.length" class="empty">暂无提现记录</view>
            <view v-for="withdrawal in store.withdrawalRecords" :key="withdrawal.id" class="withdrawal-record">
              <view><text>{{ withdrawal.method }} · {{ money(withdrawal.amount) }}</text><small>申请 {{ withdrawal.createdAt }}</small><small v-if="withdrawal.reviewedAt">审核 {{ withdrawal.reviewedAt }} · {{ withdrawal.operator || '平台' }}</small><small v-if="withdrawal.reviewedNote">备注：{{ withdrawal.reviewedNote }}</small></view>
              <span :class="withdrawal.status">{{ withdrawal.status === 'pending' ? '待审核' : withdrawal.status === 'approved' ? '已通过' : '已驳回' }}</span>
            </view>
          </view>
        </view>
        ...
        <view v-else-if="sheet === 'live' && selectedLive" class="live-detail"><BusinessImage :src="selectedLive.image" mode="aspectFit" /><view class="live-state"><span></span>前端模拟直播间</view><text>{{ selectedLive.title }}</text><small>{{ selectedLive.host }} · {{ selectedLive.viewers.toLocaleString('zh-CN') }} 人观看</small><view><strong>{{ selectedLive.productName }}</strong><b>{{ money(selectedLive.productPrice) }}</b></view><button class="primary-button" @click="shareLive">分享直播赚佣金</button></view>
        <view v-else-if="sheet === 'materials'" class="sheet-materials"><view v-if="!cityProducts.length" class="empty">当前城市暂无推广素材</view><view v-for="product in cityProducts" :key="product.id"><BusinessImage :src="product.image" mode="aspectFit" /><view><text>{{ product.name }}</text><small>预计佣金 {{ money(commissionFor(product)) }}</small></view><button @click="openShare(product)">{{ store.sharedProductIds.includes(product.id) ? '再次分享' : '生成链接' }}</button></view></view>
        ...
        <view v-else-if="sheet === 'commissions'" class="data-list"><view v-if="!store.ledgerEntries.length" class="empty">暂无佣金流水</view><view v-for="entry in store.ledgerEntries" :key="entry.id"><view class="data-avatar"><UiIcon name="badge-dollar-sign" :size="18" /></view><view><text>{{ entry.role === 'withdrawal' ? '提现' : entry.role === 'promoter' ? '推广佣金' : '门店分成' }}</text><small>{{ entry.createdAt }} · {{ entry.status === 'pending' ? '待结算' : entry.status === 'available' ? '可提现' : entry.status === 'settled' ? '已结算' : '已冲正' }}</small></view><strong :class="entry.amount < 0 ? 'withdraw' : 'income'">{{ entry.amount > 0 ? '+' : '' }}{{ money(entry.amount) }}</strong></view></view>
        ...
        <view v-else-if="sheet === 'farm-shop'" class="sheet-materials"><view v-if="!farmProducts(selectedShopFarm).length" class="empty">该门店暂无特产</view><view v-for="product in farmProducts(selectedShopFarm)" :key="product.id"><BusinessImage :src="product.image" mode="aspectFit" /><view><text>{{ product.name }}</text><small>{{ product.tags.join(' · ') }} · {{ money(product.price) }}</small></view><button @click="openShare(product)">分享赚佣金</button></view></view>
...
.discovery-head,.color-head { padding:calc(20px + env(safe-area-inset-top)) 16px 18px;background:var(--union-red);color:#fff; }.city-button { height:30px;padding:0 8px;border-radius:5px;background:rgba(255,255,255,.14);color:#fff;display:flex;align-items:center;gap:5px;font-size:13px; }.city-button .ui-icon,.color-head .ui-icon { filter:brightness(0) invert(1); }.hero-title,.hero-sub { display:block; }.hero-title { margin-top:18px;font-size:24px;font-weight:900; }.hero-sub { margin-top:5px;font-size:12px;opacity:.82; }.search { height:43px;margin-top:14px;padding:0 12px;border-radius:7px;background:#fff;display:flex;align-items:center;gap:8px;color:var(--union-ink); }.search input { flex:1;height:100%;font-size:13px; }
.filter-scroll { width:100%;white-space:nowrap;overflow-x:auto;overflow-y:hidden;background:#fff;border-bottom:1px solid var(--union-line); }.filter-row { width:max-content;padding:10px 14px;display:flex;gap:7px; }.filter-row button { height:auto;min-height:34px;padding:6px 14px;border-radius:999px;background:#fff;border:1px solid var(--union-line);color:#556655;font-size:13px!important;font-weight:600; }.filter-row button.active { background:#ffe4e9;border-color:#fbd3d9;color:#b0324c;font-weight:700; }
.map-box { height:140px;margin:12px 16px 0;border:1px solid #d6e6d4;border-radius:16px;position:relative;overflow:hidden;background:linear-gradient(135deg,#e8f1e6,#dbeede); }.grid-bg { position:absolute;inset:0;background-image:linear-gradient(#c9ddc7 1px,transparent 1px),linear-gradient(90deg,#c9ddc7 1px,transparent 1px);background-size:26px 26px;opacity:.6; }.road { position:absolute;background:#fff;border:1px solid #dde7dc; }.road-a { width:130%;height:14px;left:-40px;top:58px;transform:rotate(-8deg);opacity:.9; }.road-b { width:12px;height:140%;left:54%;top:-24px;transform:rotate(6deg);opacity:.85; }.map-pin { position:absolute;width:24px;height:24px;background:transparent;border:none;font-size:24px;line-height:24px;display:grid;place-items:center;filter:drop-shadow(0 4px 4px rgba(0,0,0,.2)); }.me { position:absolute;width:18px;height:18px;font-size:18px;line-height:18px;display:grid;place-items:center;filter:drop-shadow(0 3px 3px rgba(0,0,0,.2)); }.map-box>text { position:absolute;left:10px;bottom:8px;padding:4px 10px;background:rgba(255,255,255,.92);border-radius:8px;font-size:12px;font-weight:700;color:#3f5145; }
.section-head { padding:19px 16px 10px;display:flex;align-items:center;justify-content:space-between; }.section-head>view { display:flex;align-items:center;gap:7px; }.section-head span { width:3px;height:16px;border-radius:2px;background:var(--union-red); }.section-head text { font-size:15px;font-weight:900; }.section-head small { color:var(--union-muted);font-size:12px; }
.farm-list { padding:0 16px;display:grid;gap:10px; }.farm-card { background:#fff;border:1px solid var(--union-line);border-radius:8px;overflow:hidden; }.farm-photo { height:136px;position:relative; }.farm-photo image { width:100%;height:100%; }.farm-photo text { position:absolute;left:8px;top:8px;padding:5px 7px;border-radius:4px;background:#d59b28;color:#fff;font-size:12px;font-weight:900; }.farm-main { padding:12px; }.farm-title { display:flex;align-items:center;justify-content:space-between;gap:8px; }.farm-title>text { font-size:14px;font-weight:900; }.farm-title span { padding:4px 6px;border-radius:4px;background:#eaf5ee;color:var(--union-green);font-size:12px; }.rating { display:block;margin-top:6px;color:#8f6720;font-size:12px; }.tag-row { display:flex;gap:5px;flex-wrap:wrap;margin-top:7px; }.tag-row text { padding:4px 6px;border-radius:3px;background:#fff1e9;color:#9e5136;font-size:12px; }.farm-foot { margin-top:10px;padding-top:9px;border-top:1px solid #f0eaea;display:flex;align-items:center;justify-content:space-between; }.farm-foot small { color:var(--union-muted);font-size:12px; }.farm-foot button { height:32px;padding:0 11px;border-radius:5px;background:var(--union-red);color:#fff;display:flex;align-items:center;gap:4px;font-size:13px;font-weight:800; }.farm-foot .ui-icon { filter:brightness(0) invert(1); }
.color-head .hero-title { margin-top:8px; }.rank-tabs { margin-top:18px;padding:3px;display:grid;grid-template-columns:repeat(3,1fr);background:rgba(255,255,255,.13);border-radius:6px; }.rank-tabs button { height:35px;border-radius:4px;background:transparent;color:rgba(255,255,255,.7);font-size:13px; }.rank-tabs button.active { background:#fff;color:var(--union-red);font-weight:800; }.rank-list { margin:0 16px;display:grid;gap:10px;background:transparent;border:0;border-radius:0; }.rank-list button { width:100%;min-height:76px;padding:11px 13px;background:#fff;border:1px solid var(--union-line);border-radius:14px;display:grid;grid-template-columns:30px 46px 1fr auto;gap:12px;align-items:center;text-align:left;color:var(--union-ink); }.rank-list button:last-child { border:0; }.rank-number { width:30px;height:30px;border-radius:9px;display:grid;place-items:center;font-weight:800;font-size:15px;color:#fff; }
  ...
  .rank-number.rank-medal-n { background:#e5e9e3;color:#94a3b8; }.rank-list image { width:46px;height:46px;border-radius:6px; }.rank-list view text,.rank-list view small { display:block; }.rank-list view text { font-size:12px;font-weight:800; }.rank-list view small { margin-top:4px;color:var(--union-muted);font-size:12px; }.rank-list strong { color:var(--union-red);font-size:12px; }
.route-list { padding:0 16px;display:grid;gap:12px; }.route-card { background:#fff;border:1px solid var(--union-line);border-radius:16px;overflow:hidden;color:var(--union-ink); }.route-card>image { width:100%;height:96px; }.route-card>view { padding:10px 12px; }.route-card text,.route-card small,.route-card span { display:block; }.route-card text { font-size:14px;font-weight:900; }.route-card small { margin-top:5px;color:var(--union-muted);font-size:12px; }.route-card span { margin-top:8px;color:#8b6730;font-size:12px; }.route-card button { width:auto;height:auto;min-height:32px;margin-top:10px;padding:6px 12px;border-radius:9px;background:#fff1f2;border:1px solid #fbcad5;color:#e11d48;font-size:13px;font-weight:700; }
.live-list { padding:0 16px;display:grid;grid-template-columns:repeat(2,1fr);gap:10px; }.live-card { min-width:0;background:#fff;border:1px solid var(--union-line);border-radius:8px;overflow:hidden; }.live-cover { height:170px;position:relative; }.live-cover image { width:100%;height:100%; }.live-cover>text { position:absolute;left:7px;top:7px;padding:4px 6px;border-radius:4px;background:var(--union-red);color:#fff;font-size:12px;font-weight:800; }.live-cover>text.preview { background:#3c4145; }.live-cover small { position:absolute;right:7px;bottom:7px;padding:4px 6px;border-radius:4px;background:rgba(0,0,0,.55);color:#fff;font-size:12px; }.live-body { padding:10px; }.live-body>text,.live-body>small { display:block; }.live-body>text { min-height:34px;font-size:12px;font-weight:800;line-height:1.5; }.live-body>small { color:var(--union-muted);font-size:12px;margin-top:4px; }.live-body>view { margin-top:9px;display:flex;align-items:center;justify-content:space-between;gap:6px; }.live-body span { min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:var(--union-red);font-size:12px;font-weight:700; }.live-body button { min-width:65px;height:29px;padding:0 7px;border-radius:4px;background:var(--union-red);color:#fff;display:flex;align-items:center;justify-content:center;gap:4px;font-size:13px;font-weight:800; }.live-body button.reminded { background:var(--union-green); }.live-body .ui-icon { filter:brightness(0) invert(1); }.live-note,.lock-note { margin:15px 16px;padding:13px;background:#fff;border:1px solid var(--union-line);border-radius:8px;display:flex;align-items:flex-start;gap:9px; }.live-note text,.live-note small,.lock-note text,.lock-note small { display:block; }.live-note text,.lock-note text { font-size:12px;font-weight:800; }.live-note small,.lock-note small { margin-top:4px;color:var(--union-muted);font-size:12px;line-height:1.55; }
.promoter-page { background:#f3eeee; }.promoter-head { padding:calc(54px) 16px 18px;background:linear-gradient(135deg,#1f2937,#0f172a);color:#fff; }.profile { display:flex;align-items:center;gap:12px; }.pav { width:60px;height:60px;border-radius:16px;background:#c41e44;display:grid;place-items:center;font-size:30px; }.pname { display:block;font-size:17px!important;font-weight:800; }.plv { display:block;margin-top:2px;font-size:12px!important;opacity:.85; }.plv .vip { margin-left:4px;background:linear-gradient(135deg,#fbbf24,#f59e0b);color:#3a2a00;border-radius:5px;padding:1px 7px;font-weight:800;font-size:12px; }.wallet-card { margin:14px 16px 0;background:#fff;border:1px solid var(--union-line);border-radius:18px;padding:16px;box-shadow:0 16px 36px -28px rgba(120,20,40,.5); }.wallet-top { display:flex;justify-content:space-between;align-items:flex-end; }.wallet-top small,.wallet-top strong { display:block; }.wallet-top small { font-size:12px!important;color:var(--union-muted); }
.wallet-top .settled-tag{display:inline-block;margin-left:4px;padding:1px 5px;border-radius:4px;background:#e8f3ec;color:#17633f;font-size:12px;font-weight:700}.wallet-top strong { margin-top:4px;font-size:30px!important;font-weight:800;color:#c41e44;line-height:1.1; }.wallet-top button { height:auto;min-height:36px;padding:9px 16px;border-radius:10px;background:#c41e44;color:#fff;font-size:13px!important;font-weight:800; }.wallet-stats { margin-top:14px;display:grid;grid-template-columns:repeat(3,1fr);gap:8px;border-top:1px dashed #e4dcdc;padding-top:12px; }.wallet-stats view { text-align:center; }.wallet-stats text,.wallet-stats small { display:block; }.wallet-stats text { font-size:14px!important;font-weight:800;color:var(--union-ink); }.wallet-stats small { margin-top:3px;font-size:12px!important;color:var(--union-muted); }.promoter-tools { margin:12px 16px 0;display:grid;grid-template-columns:repeat(4,1fr);background:#fff;border:1px solid var(--union-line);border-radius:8px; }
.promoter-tools .tool-emoji{font-size:21px;line-height:1}.promoter-tools button { height:79px;background:transparent;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:7px;color:var(--union-ink);font-size:13px;border-right:1px solid #f0eaea; }.promoter-tools button:last-child { border:0; }.material-list { margin:0 16px;display:grid;gap:10px; }.material-row { display:flex;align-items:center;gap:12px;background:#fff;border:1px solid var(--union-line);border-radius:14px;padding:11px 13px; }.mat-emoji { width:42px;height:42px;border-radius:11px;flex-shrink:0;display:block; }.material-row>view:nth-child(2) { flex:1;min-width:0; }.material-row text,.material-row small { display:block; }.material-row text { font-size:13.5px!important;font-weight:700;line-height:1.4; }.material-row small { margin-top:2px;color:var(--union-muted);font-size:12px!important; }.material-row strong { color:#e11d48;font-size:12px!important;font-weight:800;white-space:nowrap;flex-shrink:0;margin-right:8px; }.material-row button { height:auto;min-height:34px;padding:6px 12px;border-radius:9px;background:#fff1f2;border:1px solid #fbcad5;color:var(--union-red);font-size:13px!important;font-weight:700;flex-shrink:0; }.material-row button.shared { background:#17633f;border-color:#17633f;color:#fff; }
.tabbar { position:fixed;left:0;right:0;bottom:0;height:calc(64px + env(safe-area-inset-bottom));padding-bottom:env(safe-area-inset-bottom);background:#fff;border-top:1px solid var(--union-line);display:grid;grid-template-columns:repeat(4,1fr);z-index:20; }.tabbar button { background:transparent;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;color:#918585;font-size:13px; }.tabbar button.active { color:var(--union-red);font-weight:800; }.tabbar button.active .ui-icon { filter:invert(29%) sepia(70%) saturate(1838%) hue-rotate(324deg); }
.preview-mask,.sheet-mask { position:fixed;inset:0;background:rgba(38,24,27,.52);z-index:40;display:flex;align-items:flex-end; }.preview,.sheet { width:100%;max-height:86vh;overflow-y:auto;background:#fff;border-radius:14px 14px 0 0;padding-bottom:calc(20px + env(safe-area-inset-bottom));position:relative; }.preview-close { position:absolute;right:12px;top:12px;width:34px;height:34px;border-radius:50%;background:rgba(255,255,255,.92);display:grid;place-items:center;z-index:2; }.preview>image { width:100%;height:220px; }.preview-body { padding:15px; }.preview-body>text,.preview-body>small { display:block; }.preview-body>text { font-size:21px;font-weight:900; }.preview-body>small { margin-top:6px;color:var(--union-muted);font-size:12px; }.preview-actions { display:grid;grid-template-columns:repeat(2,1fr);gap:9px;margin-top:14px; }.preview-actions button { height:41px;border-radius:6px;background:var(--union-red);color:#fff;display:flex;align-items:center;justify-content:center;gap:7px;font-size:13px;font-weight:800; }.preview-actions button:last-child { background:var(--union-green); }.preview-actions .ui-icon { filter:brightness(0) invert(1); }.preview-products { display:grid;grid-template-columns:repeat(2,1fr);gap:9px;margin-top:14px; }.preview-products view { border:1px solid var(--union-line);border-radius:7px;overflow:hidden;padding-bottom:9px; }.preview-products image { width:100%;height:105px; }.preview-products text,.preview-products strong { display:block;padding:0 8px; }.preview-products text { margin-top:7px;font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis; }.preview-products strong { margin-top:5px;color:var(--union-red);font-size:12px; }
.sheet { padding:8px 16px calc(22px + env(safe-area-inset-bottom)); }.sheet-handle { width:38px;height:4px;border-radius:2px;background:#ddd5d5;margin:0 auto 8px; }.sheet-head { height:47px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid #f0eaea; }.sheet-head>text { font-size:16px;font-weight:900; }.sheet-head button { width:34px;height:34px;border-radius:50%;background:#f5f0f0;display:grid;place-items:center; }.choice-list { padding-top:10px;display:grid;gap:7px; }.choice-list button { height:47px;padding:0 12px;border-radius:6px;background:#f8f4f4;color:var(--union-ink);display:flex;align-items:center;gap:9px;text-align:left;font-size:13px; }.choice-list button .ui-icon:last-child { margin-left:auto; }.choice-list button.active { background:var(--union-red-soft);color:var(--union-red);font-weight:800; }.route-detail image,.share-sheet image { width:100%;height:210px;border-radius:8px;margin-top:12px; }.route-detail text,.route-detail small,.share-sheet text,.share-sheet small { display:block; }.route-detail text,.share-sheet text { margin-top:12px;font-size:14px;font-weight:800; }.route-detail small,.share-sheet small { margin-top:7px;color:var(--union-muted);font-size:12px;line-height:1.6; }.primary-button { width:100%;height:43px;margin-top:15px;border-radius:6px;background:var(--union-red);color:#fff;font-size:13px;font-weight:800; }.primary-button:disabled { opacity:.5; }.withdraw-form { padding-top:14px; }.withdraw-form>text { display:block;color:var(--union-muted);font-size:12px;margin-bottom:12px; }.withdraw-form label span { display:block;font-size:12px;font-weight:800;margin-bottom:6px; }.withdraw-form input { width:100%;height:43px;padding:0 11px;border:1px solid var(--union-line);border-radius:6px;font-size:14px; }.choice-row { display:flex;gap:7px;margin-top:12px; }.choice-row button { flex:1;height:35px;border-radius:5px;background:#f5f0f0;color:var(--union-muted);font-size:13px; }.choice-row button.active { background:var(--union-red-soft);color:var(--union-red);font-weight:800; }
.filter-scroll{scrollbar-width:none}.filter-scroll::-webkit-scrollbar{display:none}.city-button{height:36px}.filter-row button{height:38px}.farm-foot button{height:38px}.route-card button{height:41px}.live-body button{min-width:68px;height:36px}.wallet button{height:40px}.material-row button{min-height:36px}.tabbar button{min-height:48px}.preview,.sheet{overscroll-behavior:contain}.live-detail{padding-top:12px}.live-detail>image{width:100%;height:220px;border-radius:8px}.live-state{width:max-content;margin-top:-38px;margin-left:10px;position:relative;padding:6px 9px;border-radius:4px;background:rgba(24,20,21,.72);color:#fff;font-size:12px}.live-state span{display:inline-block;width:7px;height:7px;margin-right:5px;border-radius:50%;background:#ff425b}.live-detail>text,.live-detail>small{display:block}.live-detail>text{margin-top:24px;font-size:16px;font-weight:900}.live-detail>small{margin-top:6px;color:var(--union-muted);font-size:12px}.live-detail>view:not(.live-state){margin-top:14px;padding:12px;background:#f8f4f4;border-radius:7px;display:flex;justify-content:space-between}.live-detail b{color:var(--union-red)}.sheet-materials,.data-list,.promoter-rank-list{display:grid;gap:8px;padding-top:12px}.sheet-materials>view{min-height:72px;padding:8px;background:#f8f4f4;border:1px solid var(--union-line);border-radius:7px;display:grid;grid-template-columns:56px 1fr auto;gap:9px;align-items:center}.sheet-materials image{width:56px;height:54px;border-radius:6px}.sheet-materials text,.sheet-materials small{display:block}.sheet-materials text{font-size:12px;font-weight:800;line-height:1.4}.sheet-materials small{margin-top:4px;color:var(--union-muted);font-size:12px}.sheet-materials button{min-height:38px;padding:0 10px;border-radius:5px;background:var(--union-red);color:#fff;font-size:13px;font-weight:800}.data-list>view{min-height:64px;padding:10px;background:#f8f4f4;border:1px solid var(--union-line);border-radius:7px;display:grid;grid-template-columns:36px 1fr auto;gap:9px;align-items:center}.data-avatar{width:36px;height:36px;border-radius:50%;background:var(--union-red-soft);display:grid;place-items:center}.data-list text,.data-list small{display:block}.data-list text{font-size:12px;font-weight:800}.data-list small{margin-top:5px;color:var(--union-muted);font-size:12px;line-height:1.4}.data-list span{padding:4px 6px;border-radius:4px;background:#e8f3ec;color:var(--union-green);font-size:12px}.data-list strong{font-size:12px}.data-list strong.income{color:var(--union-green)}.data-list strong.withdrawal{color:var(--union-red)}.promoter-rank-list>view{min-height:60px;padding:10px;background:#f8f4f4;border:1px solid var(--union-line);border-radius:7px;display:grid;grid-template-columns:28px 1fr auto;align-items:center;gap:8px}.promoter-rank-list b{color:var(--union-gold);font-size:17px}.promoter-rank-list text,.promoter-rank-list small{display:block}.promoter-rank-list text{font-size:12px;font-weight:800}.promoter-rank-list small{margin-top:4px;color:var(--union-muted);font-size:12px}.promoter-rank-list strong{color:var(--union-red);font-size:12px}.share-record{margin-top:12px;padding:11px;background:#f8f4f4;border:1px solid var(--union-line);border-radius:6px}
.policy-hint{margin:10px 16px 0;padding:9px 12px;border-radius:7px;background:var(--union-red-soft);border:1px solid #f6d2d8;color:var(--union-red);font-size:12px;font-weight:700}.policy-hint b{color:var(--union-red)}.share-record text,.share-record small{margin:0!important}.share-record small{margin-top:6px!important;word-break:break-all;line-height:1.5}
...
.city-button,.filter-row button,.farm-foot button,.route-card button,.live-body button,.wallet button,.material-row button,.sheet-materials button,.choice-row button{min-height:40px}.live-card:last-child:nth-child(odd){grid-column:auto}.preview-actions{grid-template-columns:repeat(3,1fr)}.preview-actions button:last-child{background:#7d4a22}.preview-actions button:disabled{opacity:.45}.booking-record-link{width:calc(100% - 32px);min-height:44px;margin:10px 16px 0;padding:0 12px;border-radius:7px;background:#fff;border:1px solid var(--union-line);display:flex;align-items:center;gap:8px;color:var(--union-ink);font-size:12px;font-weight:800}.booking-record-link .ui-icon:last-child{margin-left:auto}.state-page{min-height:100vh;padding:24px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:var(--union-muted);text-align:center}.state-page .primary-button{max-width:240px}.empty{min-height:100px;display:grid;place-items:center;color:var(--union-muted);font-size:12px}
...
/* ===== 原型 1:1 补充样式 ===== */
.switch-text{margin-left:4px;font-size:12px;color:rgba(255,255,255,.92);border:1px solid rgba(255,255,255,.4);border-radius:5px;padding:1px 6px}
.filter-row button{display:inline-flex;align-items:center;gap:4px}.filter-row button text{font-size:13px}
.rank-badge{position:absolute;left:8px;top:8px;z-index:2;padding:2px 8px;border-radius:6px;background:rgba(0,0,0,.55);color:#fff;font-size:12px;font-weight:800}.rank-badge.medal{background:#f7d9a0;color:#7a4b12}
.rank-value{display:flex;flex-direction:column;align-items:flex-end;gap:2px}.rank-value strong{font-size:15px;color:#c2562e;font-weight:800}.rank-value small{font-size:12px;color:var(--alliance-muted,#a08a8a)}
.route-meta{margin-top:3px;color:#c2562e!important}

.live-body view{flex-wrap:wrap;row-gap:6px}.live-body view span{flex:1 1 auto;min-width:0;font-size:12px;line-height:1.4}
.promoter-footer{margin:16px 16px 4px;padding-top:12px;border-top:1px dashed #e4dcdc;color:#a08a8a;font-size:12px;text-align:center;line-height:1.7}
...
.rank-tabs button{height:auto;padding:7px 15px;border-radius:999px;background:rgba(255,255,255,.2);border:1px solid rgba(255,255,255,.35);color:#fff;font-size:13px!important;font-weight:700}
...
.live-cover>text{position:absolute;top:8px;left:8px;background:var(--union-red);color:#fff;font-size:12px;font-weight:800;border-radius:6px;padding:2px 8px;line-height:1.4}
.live-cover>text.preview{background:rgba(0,0,0,.5)}
.live-cover>small{position:absolute;top:8px;right:8px;background:rgba(0,0,0,.45);color:#fff;font-size:12px;border-radius:6px;padding:2px 7px}
...
.live-body small{display:block;font-size:12px!important;color:#6f7570;margin-top:4px}
.live-body span{font-size:12px!important}
...
.withdrawal-history{display:grid;gap:8px;margin-top:18px;padding-top:14px;border-top:1px solid var(--union-line)}
.withdrawal-history-title{margin:0!important;color:var(--union-ink)!important;font-size:13px!important;font-weight:800}
.withdrawal-record{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:10px;align-items:start;padding:10px;border:1px solid var(--union-line);border-radius:7px;background:#f8f4f4}
.withdrawal-record text,.withdrawal-record small{display:block}.withdrawal-record text{font-size:12px;font-weight:800}.withdrawal-record small{margin-top:4px;color:var(--union-muted);font-size:12px;line-height:1.4}
.withdrawal-record span{padding:4px 6px;border-radius:4px;font-size:12px;white-space:nowrap}.withdrawal-record span.pending{background:#fff1d8;color:#8a5a00}.withdrawal-record span.approved{background:#e8f3ec;color:var(--union-green)}.withdrawal-record span.rejected{background:var(--union-red-soft);color:var(--union-red)}
...
.remind-row span{display:inline-flex;align-items:center;gap:4px;padding:4px 10px;border-radius:6px;background:#f3f0ee;color:#8a5a4a;font-size:12px!important;font-weight:700}
...
.login-hint{display:block;text-align:center;margin-top:14px;font-size:12px;color:#a99a9e}
...
.farm-photo .business-image,.live-cover .business-image{width:100%;height:100%}.rank-list .business-image{width:46px;height:46px;border-radius:6px}.route-card>.business-image{width:100%;height:96px}.preview>.business-image{width:100%;height:220px}.preview-products .business-image{width:100%;height:105px}.live-detail>.business-image{width:100%;height:220px;border-radius:8px}.sheet-materials .business-image{width:56px;height:54px;border-radius:6px}
