import { applyPlatformEntities, applyPlatformMedia, cloneSeed, farmhouseFoods, farms, members, mockDelay, products, selectableProducts } from '@agritainment/shared'
...
export const farmhouseRepository = {
  loadStorefront: (scenario: MockScenario = 'normal') => {
    ...
    const storeProducts = seedAll.filter((item) => item.farmIds.includes(activeTenant.farmId) && (item.source === 'farmhouse' || item.status === 'active'))
