...
function confirmAfterSale() {
  ...
  if (!store.requestSubOrderAfterSale(target.order.id, target.sub.id, afterSaleEvidence.value.length ? afterSaleEvidence.value : undefined)) return toast(store.checkoutError || '当前订单不支持售后')
  sheet.value = null
...
function buyPackage(item: Product) {
  const sku = item.skus.find((candidate) => candidate.stock > 0) || item.skus[0]
  if (!sku) return toast('该套餐暂无可售规格')
  if (!store.buyLivePackage(item.id, sku.id, 1)) return toast(store.checkoutError || '套餐购买失败')
  toast('套餐已购买，可在订单页查看')
}
...
<template>
  <view class="app-shell">
    ...
    <template v-else>
      ...
      <view v-else-if="activeTab === 'orders'" class="page-content orders-page">
        ...
        <view v-for="order in store.orders" :key="order.id" class="order-card">
          ...
          <view v-if="!isDemoOrderId(order.id)" class="order-actions"><button v-if="order.status === 'pending_payment'" class="primary-small" @click="pay(order)">模拟支付</button><button v-if="order.status === 'pending_payment'" class="outline-small" @click="store.cancelOrder(order.id) ? toast('订单已取消，库存已释放') : toast('当前订单不能取消')">取消订单</button></view>
        </view>
    ...
    <view v-if="sheet" class="sheet-mask" @click.self="sheet = null"><view class="sheet">
      ...
      <view v-else-if="sheet === 'cart'" class="cart-content"><view v-if="!store.cart.length" class="empty-block compact">购物车为空</view><view v-for="line in store.cart" :key="line.productId + line.skuId" class="cart-line"><BusinessImage :src="line.image" mode="aspectFill" /><view class="cart-line-main"><text>{{ line.name }}</text><small class="meta-text">{{ line.skuName }} · ¥{{ line.unitPrice.toFixed(2) }}</small><small v-if="line.unavailable" class="stock-warning">库存不足，请减少数量或删除</small><view class="stepper"><button @click="changeLine(line.productId, line.skuId, -1)">−</button><text>{{ line.quantity }}</text><button @click="changeLine(line.productId, line.skuId, 1)">+</button></view></view><strong class="strong-text">¥{{ (line.unitPrice * line.quantity).toFixed(2) }}</strong><button class="outline-small cart-remove" @click="store.removeFromCart(line.productId, line.skuId)">删除</button></view><view v-if="store.cart.length" class="sheet-total"><text>身份价格已锁定</text><strong class="strong-text">¥{{ store.cartTotal.toFixed(2) }}</strong></view><button v-if="store.cart.length" class="primary-btn" @click="openCheckout">去结算</button></view>
...
.live-card-status { display:flex; align-items:center; gap:5px; color:#ffe2b8; font-size:10px; font-weight:800; }
...
.live-card-sub { display:block; margin-top:4px; color:#e7efe8; font-size:10px; }
.live-card-enter { align-self:flex-start; display:flex; align-items:center; gap:5px; padding:7px 10px; border-radius:6px; background:rgba(255,255,255,.18); color:#fff; font-size:11px; font-weight:800; }
...
.live-room-meta small { display:block; margin-top:4px; color:#e7efe8; font-size:11px; }
...
.live-buy-btn { margin:0; padding:7px 11px; font-size:11px; }

.order-id-line { display:flex; align-items:center; gap:6px; min-width:0; }
.demo-badge { flex:none; padding:2px 5px; border-radius:3px; background:#eef3ef; color:#567064; font-size:9px; font-weight:700; }
