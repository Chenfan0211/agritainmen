...
describe('用户端商品分类图片', () => {
  it('使用共享品类图片替换分类系统图标', () => {
    ...
    expect(page).toMatch(/<BusinessImage\s+class="category-grid-img"\s+:src="productCategoryImage\(category, dictionaryState\)"\s+:fallback="defaultProductCategoryImage\(category\)"\s+:error-fallback="defaultProductCategoryImage\(\)"\s+:show-error="false"/)
  })
  ...
  it('圆形宫格两行对齐，分类商品卡标签与名称竖排', () => {
    expect(theme).toMatch(/\.category-grid-label\s*\{[^}]*height:\s*calc\(12px \* 1\.25 \* 2\)/s)
    expect(page).not.toContain('class="product-name-row"')
    expect(page).toMatch(/class="tag-row"[\s\S]*class="product-name"/)
    expect(page).not.toMatch(/\.tag-row[\s\S]*max-height:\s*23px/)
  })
})
