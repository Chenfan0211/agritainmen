...
import { geoFeatureRegionCode, loadDashboardRegionGeoJson, type DashboardGeoJson } from '../../region-map'
...
const activeGeoJson = ref<DashboardGeoJson | null>(null)
...
const visibleMapPointCount = computed(() => currentRegion.value?.level === 'province' ? (snapshot.value?.cityMapPoints.length || 0) : (snapshot.value?.mapPoints.length || 0))
const hasVisibleMapPoints = computed(() => visibleMapPointCount.value > 0)

watch(availableModules, (modules) => {
  if (!modules.includes(activeModule.value)) activeModule.value = 'overview'
})

watch(() => store.regionCode, async (regionCode) => {
  if (!regionCode) return
  try {
    const geoJson = await loadDashboardRegionGeoJson(regionCode)
    if (store.regionCode !== regionCode) return
    const mapName = `dashboard-region-${regionCode}`
    ...
    activeMapName.value = ''
    pageError.value = error instanceof Error ? error.message : '区域地图加载失败'
  }
}, { immediate: true })
...
const storeStatusOption = computed<EChartsOption>(() => ({
  ...
  legend: { bottom: 0, textStyle: { color: '#8da7b5', fontSize: 10 }, itemWidth: 8, itemHeight: 8 },
  series: [{
...
const fulfillmentOption = computed<EChartsOption>(() => ({
  ...
  series: [{
    ...
    anchor: { show: false }, title: { offsetCenter: [0, '50%'], color: '#8da7b5', fontSize: 10 },
...
const inventoryOption = computed<EChartsOption>(() => ({
  ...
  xAxis: { type: 'value', axisLabel: { color: '#9bb1bb', fontSize: 10 }, splitLine: { lineStyle: { color: '#193040' } } },
  yAxis: { type: 'category', data: ['低库存SKU', '可售SKU', '库存总量'], axisLabel: { color: '#a9bec8', fontSize: 10 }, axisTick: { show: false }, axisLine: { show: false } },
  ...
  ], label: { show: true, position: 'right', color: '#d9e8ec', fontSize: 10 } }]
}))

const mapOption = computed<EChartsOption>(() => {
  const isProvince = currentRegion.value?.level === 'province'
  const cityData = (snapshot.value?.cityMapPoints || []).map((point) => ({
    name: point.cityName,
    cityCode: point.cityCode,
    entityType: 'city',
    value: [point.longitude, point.latitude, point.transactionAmount],
    storeCount: point.storeCount,
    amount: point.transactionAmount,
    count: point.orderCount,
    riskCount: point.riskCount
  }))
  const statusColors = { active: '#52d681', pending: '#f6c653', paused: '#8a9ba6' }
  const labelPositions = ['left', 'right', 'bottom', 'top'] as const
  const farmData = (snapshot.value?.mapPoints || []).map((point, index) => ({
    name: point.name,
    farmId: point.farmId,
    entityType: 'farm',
    value: [point.longitude, point.latitude, point.transactionAmount],
    ...
    count: point.orderCount,
    status: point.status,
    districtName: point.regionName,
    itemStyle: { color: statusColors[point.status] },
    label: { position: labelPositions[index % labelPositions.length] }
  }))
  return {
    aria: { enabled: true, description: `${mapTitle.value}，${isProvince ? `按城市聚合农家乐门店：${cityData.map((item) => item.name).join('、')}` : `展示真实门店坐标和经营数据：${farmData.map((item) => item.name).join('、')}`}` },
    tooltip: { trigger: 'item', backgroundColor: '#102635', borderColor: '#31596d', textStyle: { color: '#e9f4f6', fontSize: 12 }, formatter: (params: unknown) => {
      const item = params as { name?: string; data?: { entityType?: string; amount?: number; count?: number; storeCount?: number; riskCount?: number; districtName?: string; status?: string } }
      if (item.data?.entityType === 'city') return `${item.name}<br/>门店 ${item.data.storeCount || 0} 家<br/>交易额 ¥${money(item.data.amount || 0)}<br/>订单 ${item.data.count || 0} 单<br/>风险 ${item.data.riskCount || 0} 项`
      if (item.data?.entityType === 'farm') return `${item.name}<br/>县区 ${item.data.districtName || '--'}<br/>状态 ${farmStatusLabel(item.data.status)}<br/>交易额 ¥${money(item.data.amount || 0)}<br/>订单 ${item.data.count || 0} 单`
      return item.name || ''
    } },
    geo: {
      map: activeMapName.value, roam: true, zoom: 1.04, selectedMode: false,
      label: { show: false, color: '#a9c3ce', fontSize: 10 },
      itemStyle: { areaColor: '#132d3b', borderColor: '#4a91a2', borderWidth: 1 },
      emphasis: { label: { color: '#ffffff', fontSize: 11 }, itemStyle: { areaColor: '#1d5060' } }
    },
    series: isProvince ? [{
      name: '城市经营聚合', type: 'effectScatter', coordinateSystem: 'geo', rippleEffect: { scale: 2.4, brushType: 'stroke' },
      symbolSize: (value: number[], params: unknown) => {
        const item = params as { data?: { storeCount?: number } }
        return Math.max(20, Math.min(38, 18 + Number(item.data?.storeCount || 0) * 3 + Math.log10(Math.max(1, Number(value[2] || 0))) * 2))
      },
      itemStyle: { color: '#ffd166', shadowBlur: 10, shadowColor: 'rgba(255,209,102,.75)' },
      label: { show: true, position: 'right', distance: 5, color: '#f2fbfd', fontSize: 11, fontWeight: 600, formatter: (params: unknown) => {
        const item = params as { name?: string; data?: { storeCount?: number } }
        return `${item.name || ''}\n${item.data?.storeCount || 0}家`
      } },
      labelLayout: { hideOverlap: true, moveOverlap: 'shiftY' }, data: cityData
    }] : [{
      name: '农家乐门店', type: 'scatter', coordinateSystem: 'geo', symbol: 'pin', symbolSize: [24, 30],
      itemStyle: { borderColor: '#f3fbfd', borderWidth: 1, shadowBlur: 8, shadowColor: 'rgba(0,0,0,.55)' },
      label: { show: true, position: 'right', distance: 6, color: '#effbfc', fontSize: 11, fontWeight: 600, formatter: '{b}' },
      labelLayout: { hideOverlap: true, moveOverlap: 'shiftY' }, emphasis: { scale: 1.2, label: { show: true } }, data: farmData
    }]
  }
})
...
const gapOption = computed<EChartsOption>(() => {
  ...
  return {
    ...
    xAxis: { type: 'value', axisLabel: { color: '#9bb1bb', fontSize: 10, formatter: '{value}%' }, splitLine: { lineStyle: { color: '#193040' } } },
    yAxis: { type: 'category', data: items.map((item) => item.category), axisLabel: { color: '#a9bec8', fontSize: 10, width: 58, overflow: 'truncate' }, axisTick: { show: false }, axisLine: { show: false } },
    series: [{ type: 'bar', barWidth: 8, data: items.map((item) => ({ value: item.gap, itemStyle: { color: item.gap > 20 ? '#f3b33d' : item.gap < 0 ? '#48a8ff' : '#24d4c4' } })), label: { show: true, position: 'right', formatter: '{c}%', color: '#e3f0f3', fontSize: 10 } }]
  }
})
...
const trendOption = computed<EChartsOption>(() => ({
  ...
  xAxis: { type: 'category', boundaryGap: false, data: (snapshot.value?.trend || []).map((item) => item.date.slice(5)), axisLabel: { color: '#9bb1bb', fontSize: 10 }, axisLine: { lineStyle: { color: '#264252' } }, axisTick: { show: false } },
  yAxis: [
    { type: 'value', axisLabel: { color: '#9bb1bb', fontSize: 10 }, splitLine: { lineStyle: { color: '#193040' } } },
    { type: 'value', axisLabel: { color: '#9bb1bb', fontSize: 10 }, splitLine: { show: false } }
  ],
  series: [
...
function handleMapSelect(params: { componentType?: string; seriesType?: string; name?: string; data?: Record<string, unknown> }) {
  if (params.componentType === 'series' && params.data?.entityType === 'city' && typeof params.data.cityCode === 'string') {
    navigateRegion(params.data.cityCode)
    return
  }
  if (params.componentType === 'series' && params.data?.entityType === 'farm' && typeof params.data.farmId === 'string') {
    const farm = snapshot.value?.farmProfiles.find((item) => item.farmId === params.data?.farmId)
    if (farm) {
  ...
  if (params.componentType !== 'geo' || !params.name || !activeGeoJson.value || currentRegion.value?.level === 'district') return
  const code = geoFeatureRegionCode(activeGeoJson.value, params.name)
  if (code) navigateRegion(code)
}
  ...
  <view class="dashboard-shell">
    <header class="topbar">
      ...
      <view class="toolbar">
        ...
        <view class="principal-badge"><small>{{ store.principal?.role === 'leader' ? '领导' : store.principal?.role === 'regulator' ? '监管' : '产业服务' }}</small><strong>{{ store.principal?.name }}</strong></view>
        <button class="icon-button" role="button" aria-label="退出登录" title="退出登录" @click="logout"><img :src="logoutIcon" alt="" /></button>
      </view>
    </header>
    ...
    <main class="main-grid">
      ...
      <article class="panel map-panel">
        ...
        <view class="map-chart"><DashboardChart v-if="activeMapName && hasVisibleMapPoints" ref="mapChart" id="farm-map-chart" :option="mapOption" :aria-label="mapTitle" @select="handleMapSelect" @error="handleChartError" /><view v-else-if="activeMapName" class="empty-state">当前区域暂无已定位门店</view><view v-else class="empty-state">区域地图暂不可用</view></view>
      ...
      <section class="side-column right-column">
        <article class="panel risk-panel">
          ...
          <view class="panel-body list-body">
            ...
            <button v-for="risk in topRisks" :key="risk.id" class="risk-row" :class="`risk-${risk.level}`" @click="openRisk(risk, $event)">
    ...
    <main v-else class="module-workspace" :data-module-view="activeModule">
      <template v-if="activeModule === 'regional-comparison'">
        ...
        <section class="module-panel module-map-panel">
          ...
          <DashboardChart v-if="activeMapName && hasVisibleMapPoints" ref="mapChart" id="regional-map-chart" :option="mapOption" :aria-label="mapTitle" @select="handleMapSelect" @error="handleChartError" /><view v-else class="empty-state">当前区域暂无已定位门店</view>
        </section>
      ...
      <template v-else-if="activeModule === 'regulatory-monitoring'">
        <section class="module-panel regulatory-list-panel">
          ...
          <view class="regulatory-scroll">
            ...
            <button v-for="risk in orderedRisks" :key="risk.id" class="risk-table risk-table-button" @click="openRisk(risk, $event)">
    ...
    <view v-if="store.lastError || pageError" class="error-toast" role="alert">{{ snapshot ? '当前展示缓存数据 · ' : '' }}{{ store.lastError || pageError }}<span v-if="snapshot && store.lastSuccessfulAt"> · 最后成功 {{ new Date(store.lastSuccessfulAt).toLocaleString('zh-CN', { hour12: false }) }}</span></view>

    <view v-if="snapshot && !hasBusinessData" class="empty-business" role="status">当前授权范围暂无业务数据</view>

    <view v-if="showDefinitions" class="modal-mask" @click="closeOverlay">
    ...
    <view v-if="selectedRisk" class="drawer-mask" @click="closeOverlay">
    ...
    <view v-if="selectedFarm" class="drawer-mask" @click="closeOverlay">
...
.dashboard-shell {
  --bg: #07111c; --panel: #0c1b28; --line: #1e3c4d; --muted: #7893a2; --text: #e9f4f6;
  width: 100vw; height: 100vh; min-width: 1180px; min-height: 680px; overflow: hidden; padding: 10px 12px;
  display: grid; grid-template-rows: 56px 100px minmax(0, 1fr) 208px; gap: 9px;
  color: var(--text); background-color: var(--bg); background-image: linear-gradient(rgba(33, 67, 84, .12) 1px, transparent 1px), linear-gradient(90deg, rgba(33, 67, 84, .1) 1px, transparent 1px); background-size: 32px 32px;
}
.topbar { display: grid; grid-template-columns: minmax(310px, 1fr) auto minmax(470px, 1fr); align-items: center; border-bottom: 1px solid #1d4051; }
.brand-block { display: flex; align-items: center; gap: 11px; min-width: 0; }
.brand-mark { width: 32px; height: 32px; display: grid; grid-template-columns: repeat(3, 1fr); gap: 3px; align-items: end; border: 1px solid #2e6c78; padding: 6px; }
...
.brand-block h1 { font-size: 20px; line-height: 1.2; margin: 0; font-weight: 700; white-space: nowrap; }.brand-block p { color: var(--muted); font-size: 10px; margin: 4px 0 0; }
.module-nav { display: flex; align-items: stretch; height: 38px; border: 1px solid #1d4051; }
.module-nav button { min-width: 86px; padding: 0 14px; margin: 0; border: 0; border-radius: 0; color: #8fa8b4; background: #0b1824; font-size: 12px; line-height: 36px; cursor: pointer; }
...
.toolbar select { height: 32px; color: #bdd0d7; background: #0b1b28; border: 1px solid #264757; padding: 0 25px 0 9px; font-size: 11px; outline: none; }
.data-stale,.data-unknown { height: 32px; padding: 0 8px; display: flex; align-items: center; font-size: 9px; white-space: nowrap; }.data-stale { border: 1px solid #8d6230; color: #f3b33d; background: #241d18; }.data-unknown { border: 1px solid #456879; color: #9fc4d2; background: #10212c; }
.principal-badge { min-width: 118px; height: 32px; display: flex; flex-direction: column; justify-content: center; padding: 0 9px; border-left: 2px solid #24d4c4; background: #0b1b28; }.principal-badge small { color: #24d4c4; font-size: 8px; }.principal-badge strong { color: #dcecef; font-size: 10px; font-weight: 600; white-space: nowrap; }
.icon-button { width: 32px; height: 32px; padding: 7px; margin: 0; border-radius: 0; border: 1px solid #264757; background: #0b1b28; cursor: pointer; display: grid; place-items: center; }
...
.kpi { --tone: #24d4c4; position: relative; overflow: hidden; border: 1px solid #1c3949; border-top: 2px solid var(--tone); background: #0c1b28; padding: 10px 14px 8px; }
.kpi::after { content: ''; position: absolute; right: -10px; bottom: -28px; width: 70px; height: 70px; border: 1px solid color-mix(in srgb, var(--tone), transparent 65%); transform: rotate(45deg); }
...
.kpi-heading { display: flex; justify-content: space-between; align-items: center; color: #8da7b5; font-size: 11px; }.kpi-heading i { width: 6px; height: 6px; background: var(--tone); }
.kpi-value { display: flex; align-items: baseline; gap: 5px; margin-top: 5px; color: var(--tone); white-space: nowrap; }.kpi-value strong { font-family: Bahnschrift, Arial, sans-serif; font-size: 27px; font-weight: 600; }.kpi-value small { color: #9fb4be; font-size: 10px; }
.kpi-note { color: #66818f; font-size: 9px; margin-top: 2px; }
.main-grid { min-height: 0; display: grid; grid-template-columns: minmax(250px, 22%) minmax(500px, 1fr) minmax(290px, 24%); gap: 9px; }
.side-column { min-height: 0; display: grid; grid-template-rows: repeat(3, minmax(0, 1fr)); gap: 9px; }
.panel { min-width: 0; min-height: 0; overflow: hidden; border: 1px solid var(--line); background: var(--panel); position: relative; }
.panel::before, .panel::after { content: ''; position: absolute; width: 12px; height: 12px; pointer-events: none; z-index: 2; }.panel::before { left: -1px; top: -1px; border-left: 2px solid #2b8090; border-top: 2px solid #2b8090; }.panel::after { right: -1px; bottom: -1px; border-right: 2px solid #2b8090; border-bottom: 2px solid #2b8090; }
...
.panel-header h2 { margin: 0; color: #d8e8ec; font-size: 12px; font-weight: 600; }.panel-header span { color: #668492; font-size: 9px; }.panel-body { height: calc(100% - 32px); min-height: 0; }.chart-body { padding: 2px 4px 4px; }
.map-panel { display: grid; grid-template-rows: 40px 48px minmax(0, 1fr) 24px; }
.map-header { height: 40px; padding: 0 14px; }.map-header > view:first-child { display: flex; align-items: center; gap: 10px; }.map-header h2 { font-size: 14px; }.map-heading { min-width: 0; flex: 1; justify-content: space-between; }.map-heading > view { display: flex; align-items: baseline; gap: 10px; }
.region-breadcrumb { display: flex; align-items: center; gap: 3px; }.region-breadcrumb button { min-width: 0; height: 22px; margin: 0; padding: 0 5px; border: 0; border-radius: 0; color: #7ea0ae; background: transparent; font-size: 9px; cursor: pointer; }.region-breadcrumb button + button::before { content: '/'; margin-right: 6px; color: #3d6170; }.region-breadcrumb button:disabled { color: #dcebed; cursor: default; }
.map-legend { display: flex; gap: 12px; }.map-legend span { color: #829ba8; font-size: 9px; }.dot { display: inline-block; width: 6px; height: 6px; margin-right: 4px; }.dot.located { background: #ffd166; box-shadow: 0 0 7px #ffd166; }.dot.missing { border: 1px solid #f26464; }
.map-summary { position: relative; z-index: 3; display: flex; justify-content: center; gap: 8px; padding-top: 7px; }
.map-summary view { min-width: 112px; height: 36px; padding: 5px 9px; border-left: 2px solid #2b8090; background: #102432; }.map-summary small { color: #7793a1; font-size: 9px; }.map-summary strong { float: right; color: #dff7f6; font: 600 17px Bahnschrift, Arial; }
.map-chart { min-height: 0; padding: 0 6px; }.map-footer { display: flex; justify-content: space-between; padding: 0 12px; color: #5c7785; font-size: 8px; line-height: 22px; border-top: 1px solid #162e3d; }
.list-body { padding: 4px 8px; overflow: auto; }.risk-row { width: 100%; min-height: 30px; margin: 0; padding: 0; display: grid; grid-template-columns: 40px 1fr 20px; gap: 6px; align-items: center; border: 0; border-bottom: 1px solid #17303f; border-radius: 0; color: inherit; background: transparent; text-align: left; cursor: pointer; }.risk-row:last-child { border-bottom: 0; }.risk-row:hover { background: #102735; }
.risk-level { width: 36px; padding: 2px 0; text-align: center; font-size: 8px; border: 1px solid #c38a30; color: #f3b33d; }.risk-critical .risk-level { border-color: #b94b53; color: #ff7b7b; }.risk-notice .risk-level { border-color: #3a7189; color: #62bde3; }
.risk-row view { min-width: 0; }.risk-row strong, .risk-row small { display: block; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.risk-row strong { color: #cfe0e5; font-size: 10px; }.risk-row small { color: #6e8997; font-size: 8px; margin-top: 2px; }.risk-row b { color: #f26464; font: 600 14px Bahnschrift, Arial; text-align: right; }.danger-text { color: #f26464 !important; }
.rank-list { padding: 5px 9px; }.rank-row { height: 25px; display: grid; grid-template-columns: 16px 55px 1fr 66px 30px; align-items: center; gap: 5px; font-size: 9px; }.rank-row > b { color: #f3b33d; font: 600 11px Bahnschrift; }.rank-row > span { color: #a9bdc5; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.rank-track { height: 4px; background: #1b3443; }.rank-track i { display: block; height: 100%; background: #24d4c4; }.rank-row > strong { color: #d7e7eb; font-size: 9px; text-align: right; }.rank-row > small { color: #668492; font-size: 8px; text-align: right; }
.bottom-grid { min-height: 0; display: grid; grid-template-columns: 36% 31% 1fr; gap: 9px; }
.chain-flow { height: 105px; display: flex; align-items: center; justify-content: center; padding: 8px; }.chain-flow > view { width: 76px; height: 68px; display: flex; flex-direction: column; justify-content: center; text-align: center; border: 1px solid #2b596b; background: #0f2633; }.chain-flow small { color: #7f9aa7; font-size: 8px; }.chain-flow strong { color: #24d4c4; font: 600 21px Bahnschrift; margin: 3px 0; }.chain-flow span { color: #587583; font-size: 8px; }.chain-flow > i { width: 22px; height: 1px; background: #3b7181; position: relative; }.chain-flow > i::after { content: ''; position: absolute; right: 0; top: -3px; border-left: 5px solid #3b7181; border-top: 3px solid transparent; border-bottom: 3px solid transparent; }
.chain-metrics { height: 52px; margin: 0 10px; border-top: 1px solid #193647; display: flex; align-items: center; justify-content: space-around; color: #718c99; font-size: 9px; }.chain-metrics b { color: #d8e8ec; margin-left: 4px; font: 600 12px Bahnschrift; }
.suggestion-list { padding: 5px 8px; height: calc(100% - 32px); overflow: auto; }.suggestion-row { min-height: 49px; display: grid; grid-template-columns: 25px 1fr 105px; align-items: center; gap: 7px; border-bottom: 1px solid #17303f; }.suggestion-row:last-child { border-bottom: 0; }.suggestion-row > b { color: #24d4c4; font: 600 15px Bahnschrift; }.suggestion-row view { min-width: 0; }.suggestion-row strong, .suggestion-row small { display: block; overflow: hidden; white-space: nowrap; text-overflow: ellipsis; }.suggestion-row strong { color: #d5e5e9; font-size: 10px; }.suggestion-row small { color: #6a8593; font-size: 8px; margin-top: 3px; }.suggestion-row > span { color: #f3b33d; font-size: 8px; line-height: 1.35; text-align: right; }
.empty-state { height: 100%; display: grid; place-items: center; color: #67818e; font-size: 10px; }.error-toast { position: fixed; left: 50%; top: 72px; transform: translateX(-50%); z-index: 30; padding: 9px 14px; color: #fff; background: #9b3c45; font-size: 11px; }.page-state,.empty-business { position: fixed; inset: 0; z-index: 28; display: grid; place-items: center; color: #9fbbc5; background: rgba(7,17,28,.88); font-size: 13px; }.page-state-error { color: #ff9b9b; }.empty-business { inset: 166px 12px 10px; z-index: 5; background: rgba(7,17,28,.72); pointer-events: none; }
...
.module-panel { min-width: 0; min-height: 0; overflow: hidden; border: 1px solid var(--line); background: var(--panel); position: relative; }.module-panel::before { content: ''; position: absolute; left: -1px; top: -1px; z-index: 3; width: 14px; height: 14px; border-left: 2px solid #2b8090; border-top: 2px solid #2b8090; pointer-events: none; }
.module-heading { height: 54px; padding: 0 16px; display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid #193647; background: #0d202d; }.module-heading small { display: block; color: #527582; font-size: 8px; }.module-heading h2 { margin: 3px 0 0; color: #dcebed; font-size: 14px; letter-spacing: 0; }.module-heading > span { color: #7795a2; font-size: 9px; }
...
.region-table { min-height: 38px; display: grid; grid-template-columns: 1.5fr .6fr 1fr .6fr .7fr .7fr; align-items: center; gap: 8px; padding: 0 14px; border: 0; border-bottom: 1px solid #17303f; border-radius: 0; color: #9cb2bc; background: transparent; font-size: 10px; text-align: left; }.region-table.table-head { min-height: 34px; color: #5f7d8a; background: #091722; }.region-table-scroll { height: calc(100% - 88px); overflow: auto; }.region-table-scroll button { width: 100%; margin: 0; cursor: pointer; }.region-table-scroll button:hover { background: #102836; }.region-table strong { color: #d1e2e6; }.region-table b { color: #6fce83; text-align: right; }.region-table b.negative { color: #f26464; }
.regulatory-list-panel { grid-column: span 8; grid-row: span 2; }.regulatory-metrics-panel, .settlement-panel { grid-column: span 4; }.risk-table { width: 100%; min-height: 48px; margin: 0; display: grid; grid-template-columns: 64px minmax(0, 1fr) 150px 48px; align-items: center; gap: 10px; padding: 0 15px; border: 0; border-bottom: 1px solid #17303f; border-radius: 0; color: #8ea6b1; background: transparent; font-size: 10px; text-align: left; }.risk-table.table-head { min-height: 34px; color: #5f7d8a; background: #091722; }.risk-table-button { cursor: pointer; }.risk-table-button:hover { background: #102735; }.regulatory-scroll { height: calc(100% - 88px); overflow: auto; }.risk-table > b { width: 48px; padding: 3px 0; border: 1px solid #ad7f36; color: #f3b33d; text-align: center; font-size: 8px; }.risk-table > b.level-critical { border-color: #a9474f; color: #ff7777; }.risk-table > b.level-notice { border-color: #39738b; color: #62bde3; }.risk-table view { min-width: 0; }.risk-table strong, .risk-table small { display: block; }.risk-table strong { color: #d3e3e7; }.risk-table small { margin-top: 4px; color: #6d8996; }.risk-table code { overflow: hidden; color: #638998; text-overflow: ellipsis; white-space: nowrap; }.risk-table > span { color: #f26464; font: 600 16px Bahnschrift; text-align: right; }
.control-grid { height: calc(100% - 54px); display: grid; grid-template-columns: repeat(2, 1fr); }.control-grid > view { min-width: 0; display: flex; flex-direction: column; justify-content: center; padding: 12px 16px; border-right: 1px solid #17303f; border-bottom: 1px solid #17303f; }.control-grid small { color: #74909d; font-size: 9px; }.control-grid strong { margin: 5px 0 3px; color: #f3b33d; font: 600 25px Bahnschrift; }.control-grid span { color: #526f7c; font-size: 8px; }.settlement-metrics { height: calc(100% - 94px); display: grid; grid-template-columns: repeat(2, 1fr); }.settlement-metrics > view { display: flex; flex-direction: column; justify-content: center; padding: 16px; border-right: 1px solid #17303f; }.settlement-metrics small { color: #74909d; font-size: 9px; }.settlement-metrics strong { margin-top: 8px; color: #24d4c4; font: 600 22px Bahnschrift; }.settlement-note { height: 40px; padding: 0 16px; border-top: 1px solid #17303f; color: #6e8996; font-size: 9px; line-height: 40px; }
.empowerment-gap-panel { grid-column: span 4; grid-row: span 2; }.empowerment-list-panel, .empowerment-chain-panel { grid-column: span 8; }.empowerment-scroll { height: calc(100% - 54px); overflow: auto; }.empowerment-row { min-height: 70px; display: grid; grid-template-columns: 38px minmax(0, 1fr) 180px; align-items: center; gap: 12px; padding: 8px 16px; border-bottom: 1px solid #17303f; }.empowerment-row > b { color: #24d4c4; font: 600 18px Bahnschrift; }.empowerment-row view { min-width: 0; }.empowerment-row small { color: #527582; font-size: 8px; }.empowerment-row strong { display: block; margin-top: 3px; color: #d3e4e8; font-size: 12px; }.empowerment-row p { margin: 4px 0 0; overflow: hidden; color: #7894a0; font-size: 9px; text-overflow: ellipsis; white-space: nowrap; }.empowerment-row > span { color: #f3b33d; font-size: 9px; line-height: 1.5; text-align: right; }.empowerment-chain { height: calc(100% - 104px); display: flex; align-items: center; justify-content: center; }.empowerment-chain > view { width: 126px; height: 82px; display: flex; flex-direction: column; align-items: center; justify-content: center; border: 1px solid #2b596b; background: #0f2633; }.empowerment-chain small { color: #7894a0; font-size: 9px; }.empowerment-chain strong { margin-top: 7px; color: #24d4c4; font: 600 25px Bahnschrift; }.empowerment-chain > i { width: 42px; height: 1px; background: #3b7181; }.opportunity-strip { height: 50px; display: flex; align-items: center; justify-content: space-around; border-top: 1px solid #193647; color: #718c99; font-size: 9px; }.opportunity-strip b { margin-left: 6px; color: #d8e8ec; font: 600 13px Bahnschrift; }
.modal-mask { position: fixed; inset: 0; z-index: 40; display: grid; place-items: center; background: rgba(2, 8, 13, .78); }.definition-dialog { width: min(620px, 80vw); max-height: 76vh; overflow: auto; border: 1px solid #356579; background: #0b1b28; box-shadow: 0 18px 70px rgba(0,0,0,.45); }.definition-dialog header { height: 64px; padding: 0 18px; display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid #234353; }.definition-dialog h2 { margin: 0; font-size: 17px; }.definition-dialog p { margin: 5px 0 0; color: #7893a2; font-size: 10px; }.definition-dialog button { width: 30px; height: 30px; margin: 0; padding: 0; border-radius: 0; border: 1px solid #325467; color: #bdd0d7; background: transparent; line-height: 28px; cursor: pointer; }.definition-dialog dl { margin: 0; padding: 8px 18px 18px; }.definition-dialog dt { margin-top: 12px; color: #24d4c4; font-size: 12px; }.definition-dialog dd { margin: 5px 0 0; color: #9fb5bf; font-size: 11px; line-height: 1.7; }
.drawer-mask { position: fixed; inset: 0; z-index: 42; background: rgba(2,8,13,.72); }.risk-drawer { position: absolute; top: 0; right: 0; width: min(520px, 90vw); height: 100%; outline: 0; border-left: 1px solid #356579; background: #0b1b28; box-shadow: -18px 0 70px rgba(0,0,0,.45); }.risk-drawer > header { min-height: 116px; padding: 22px 24px; display: flex; justify-content: space-between; gap: 18px; border-bottom: 1px solid #234353; }.risk-drawer header small { color: #5f8291; font-size: 9px; }.risk-drawer h2 { margin: 6px 0; font-size: 20px; }.risk-drawer header p { margin: 0; color: #8ba3ad; font-size: 11px; line-height: 1.6; }.risk-drawer header button { width: 32px; height: 32px; flex: 0 0 32px; border: 1px solid #325467; border-radius: 0; color: #bdd0d7; background: transparent; cursor: pointer; }.risk-drawer-summary { height: 74px; display: flex; align-items: baseline; gap: 10px; padding: 18px 24px; border-bottom: 1px solid #193647; }.risk-drawer-summary span { padding: 3px 7px; border: 1px solid #ad7f36; color: #f3b33d; font-size: 9px; }.risk-drawer-summary span.level-critical { border-color: #a9474f; color: #ff7777; }.risk-drawer-summary strong { color: #f26464; font: 600 28px Bahnschrift; }.risk-drawer-summary small { color: #718d99; }.risk-subject-list { height: calc(100% - 190px); overflow: auto; padding: 8px 24px 24px; }.risk-subject-list article { padding: 15px 0; border-bottom: 1px solid #193647; }.risk-subject-list article > view { display: flex; justify-content: space-between; gap: 12px; }.risk-subject-list strong { color: #d8e8ec; font-size: 12px; }.risk-subject-list span,.risk-subject-list small { color: #5f7f8d; font-size: 9px; }.risk-subject-list p { margin: 8px 0; color: #8fa7b1; font-size: 10px; line-height: 1.6; }
@media (max-height: 820px) {
  .dashboard-shell { grid-template-rows: 50px 88px minmax(0, 1fr) 174px; gap: 7px; padding: 7px 9px; }
...
.dashboard-shell { font-family: "Microsoft YaHei", "PingFang SC", "Noto Sans CJK SC", system-ui, sans-serif; font-size: 11px; }
.topbar { grid-template-columns: 310px auto minmax(0, 1fr); }
.brand-block h1 { font-size: 21px; }.brand-block p { color: #a3bac4; font-size: 11px; }
.toolbar { min-width: 0; }.toolbar select { max-width: 104px; font-size: 11px; }.toolbar select:disabled { color: #718b97; }
.data-status { height: 32px; padding: 0 8px; display: flex; align-items: center; border: 1px solid #3d6577; color: #c6dce3; background: #10212c; font-size: 11px; white-space: nowrap; }
...
.principal-badge small,.principal-badge strong { font-size: 11px; }
.kpi-heading,.kpi-note,.kpi-value small { color: #a9bec7; font-size: 11px; }.kpi-value strong { font-size: 30px; font-variant-numeric: tabular-nums; }
.panel-header h2 { font-size: 14px; }.panel-header span,.region-breadcrumb button,.map-legend span,.map-summary small,.map-footer { color: #96adb7; font-size: 11px; }
.map-actions { display: flex; align-items: center; gap: 8px; }.map-legend { display: flex; gap: 9px; align-items: center; }.dot { width: 8px; height: 8px; border-radius: 50%; }.dot.city { background: #ffd166; box-shadow: 0 0 7px #ffd166; }.dot.active { background: #52d681; }.dot.pending { background: #f6c653; }.dot.paused { background: #8a9ba6; }.dot.missing { border-color: #ff7b7b; }
.map-reset { width: 28px; height: 28px; flex: 0 0 28px; margin: 0; padding: 6px; display: grid; place-items: center; border: 1px solid #365b6c; border-radius: 0; background: #102432; cursor: pointer; }.map-reset img { width: 15px; height: 15px; filter: invert(91%) sepia(12%) saturate(301%) hue-rotate(147deg); }
.map-footer { color: #8ea7b2; }
.risk-level,.risk-row strong,.risk-row small,.rank-row,.rank-row > strong,.rank-row > small,.chain-flow small,.chain-flow span,.chain-metrics,.suggestion-row strong,.suggestion-row small,.suggestion-row > span,.empty-state { font-size: 11px; }
...
.module-heading small,.module-heading > span,.region-table,.risk-table,.risk-table > b,.control-grid small,.control-grid span,.settlement-metrics small,.settlement-note,.empowerment-row small,.empowerment-row p,.empowerment-row > span,.empowerment-chain small,.opportunity-strip { font-size: 11px; }
...
.farm-status-strip { height: 54px; padding: 0 24px; display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid #234353; }.farm-status-strip span { padding: 4px 8px; border: 1px solid #3a7564; color: #79dfa5; }.farm-status-strip .farm-status-pending { border-color: #8d6d30; color: #f6c653; }.farm-status-strip .farm-status-paused { border-color: #61737d; color: #aab9c0; }.farm-status-strip small { color: #ffd77b; font-size: 11px; }
.farm-detail-grid { display: grid; grid-template-columns: repeat(2, 1fr); border-bottom: 1px solid #234353; }.farm-detail-grid > view { min-height: 96px; padding: 18px 24px; display: flex; flex-direction: column; justify-content: center; border-right: 1px solid #193647; border-bottom: 1px solid #193647; }.farm-detail-grid small,.farm-detail-grid span { color: #93aab5; font-size: 11px; }.farm-detail-grid strong { margin: 6px 0 3px; color: #e3f0f3; font: 600 22px Bahnschrift, Arial; }
.farm-location-detail { margin: 0; padding: 18px 24px; }.farm-location-detail dt { margin-top: 12px; color: #24d4c4; font-size: 12px; }.farm-location-detail dd { margin: 6px 0 0; color: #b0c3ca; font-size: 11px; line-height: 1.6; }
@media (max-height: 820px) {
  .topbar { grid-template-columns: 260px auto minmax(0, 1fr); }.brand-block h1 { font-size: 20px; }.kpi-value strong { font-size: 28px; }
  .toolbar { gap: 4px; }.toolbar select { max-width: 88px; padding-left: 6px; }.principal-badge { min-width: 100px; }.map-legend { gap: 6px; }
}
@media (max-width: 1450px) {
  .topbar { grid-template-columns: 260px auto minmax(0, 1fr); }.brand-block h1 { font-size: 20px; }.kpi-value strong { font-size: 28px; }
  .toolbar { gap: 4px; }.toolbar select { max-width: 88px; padding-left: 6px; }.principal-badge { min-width: 100px; }.module-nav button { min-width: 72px; padding: 0 8px; }
}
</style>
