...
const mapOption = computed<EChartsOption>(() => {
  ...
  return {
    ...
    tooltip: { trigger: 'item', backgroundColor: '#102635', borderColor: '#31596d', textStyle: { color: '#e9f4f6', fontSize: UI_TYPOGRAPHY.chartTooltip }, formatter: (params: unknown) => {
    ...
    geo: {
      ...
      label: { show: false, color: '#a9c3ce', fontSize: UI_TYPOGRAPHY.chartLabel },
      itemStyle: { areaColor: '#132d3b', borderColor: '#4a91a2', borderWidth: 1 },
      emphasis: { label: { color: '#ffffff', fontSize: UI_TYPOGRAPHY.chartLabel }, itemStyle: { areaColor: '#1d5060' } }
    },
    series: isProvince ? [{
      ...
      label: { show: false, position: 'right', distance: 5, color: '#f2fbfd', fontSize: UI_TYPOGRAPHY.chartLabel, fontWeight: 600, formatter: (params: unknown) => {
        const item = params as { name?: string; data?: { storeCount?: number } }
        return `${item.name || ''}\n${item.data?.storeCount || 0}家`
      } },
      labelLayout: { hideOverlap: true, moveOverlap: 'shiftY' }, emphasis: { scale: 1.15, label: { show: true } }, data: cityData
    }] : [{
      ...
      label: { show: true, position: 'right', distance: 6, color: '#effbfc', fontSize: UI_TYPOGRAPHY.chartLabel, fontWeight: 600, formatter: '{b}' },
...
.dashboard-shell {
  --bg: #07111c; --panel: #0c1b28; --line: #1e3c4d; --muted: #8aa4b1; --text: #e9f4f6;
...
.brand-block h1 { font-size: 20px; line-height: 1.2; margin: 0; font-weight: 700; white-space: nowrap; }.brand-block p { color: var(--muted); font-size: 11px; margin: 4px 0 0; }
.module-nav { display: flex; align-items: stretch; height: 38px; border: 1px solid #1d4051; }
.module-nav button { min-width: 86px; padding: 0 14px; margin: 0; border: 0; border-radius: 0; color: #8fa8b4; background: #0b1824; font-size: 13px; line-height: 36px; cursor: pointer; }
...
.toolbar select { height: 32px; color: #bdd0d7; background: #0b1b28; border: 1px solid #264757; padding: 0 25px 0 9px; font-size: 13px; outline: none; }
.data-stale,.data-unknown { height: 32px; padding: 0 8px; display: flex; align-items: center; font-size: 11px; white-space: nowrap; }.data-stale { border: 1px solid #8d6230; color: #f3b33d; background: #241d18; }.data-unknown { border: 1px solid #456879; color: #9fc4d2; background: #10212c; }
.principal-badge { min-width: 118px; height: 32px; display: flex; flex-direction: column; justify-content: center; padding: 0 9px; border-left: 2px solid #24d4c4; background: #0b1b28; }.principal-badge small { color: #24d4c4; font-size: 12px; }.principal-badge strong { color: #dcecef; font-size: 11px; font-weight: 600; white-space: nowrap; }
...
.panel-header h2 { margin: 0; color: #d8e8ec; font-size: 12px; font-weight: 600; }.panel-header span { color: #668492; font-size: 11px; }.panel-body { height: calc(100% - 32px); min-height: 0; }.chart-body { padding: 2px 4px 4px; }
...
.map-legend { display: flex; gap: 12px; }.map-legend span { color: #829ba8; font-size: 11px; }.dot { display: inline-block; width: 6px; height: 6px; margin-right: 4px; }.dot.located { background: #ffd166; box-shadow: 0 0 7px #ffd166; }.dot.missing { border: 1px solid #f26464; }
.map-summary { position: relative; z-index: 3; display: flex; justify-content: center; gap: 8px; padding-top: 7px; }
.map-summary view { min-width: 112px; height: 36px; padding: 5px 9px; border-left: 2px solid #2b8090; background: #102432; }.map-summary small { color: #7793a1; font-size: 12px; }.map-summary strong { float: right; color: #dff7f6; font: 600 17px var(--font-numeric); }
...
.risk-level { width: 36px; padding: 2px 0; text-align: center; font-size: 11px; border: 1px solid #c38a30; color: #f3b33d; }.risk-critical .risk-level { border-color: #b94b53; color: #ff7b7b; }.risk-notice .risk-level { border-color: #3a7189; color: #62bde3; }
.risk-row view { min-width: 0; }.risk-row strong, .risk-row small { display: block; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.risk-row strong { color: #cfe0e5; font-size: 11px; }.risk-row small { color: #6e8997; font-size: 12px; margin-top: 2px; }.risk-row b { color: #f26464; font: 600 14px var(--font-numeric); text-align: right; }.danger-text { color: #f26464 !important; }
.rank-list { padding: 5px 9px; }.rank-row { height: 25px; display: grid; grid-template-columns: 16px 55px 1fr 66px 30px; align-items: center; gap: 5px; font-size: 11px; }.rank-row > b { color: #f3b33d; font: 600 11px var(--font-numeric); }.rank-row > span { color: #a9bdc5; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.rank-track { height: 4px; background: #1b3443; }.rank-track i { display: block; height: 100%; background: #24d4c4; }.rank-row > strong { color: #d7e7eb; font-size: 11px; text-align: right; }.rank-row > small { color: #668492; font-size: 12px; text-align: right; }
.bottom-grid { min-height: 0; display: grid; grid-template-columns: 36% 31% 1fr; gap: 9px; }
.chain-flow { height: 105px; display: flex; align-items: center; justify-content: center; padding: 8px; }.chain-flow > view { width: 76px; height: 68px; display: flex; flex-direction: column; justify-content: center; text-align: center; border: 1px solid #2b596b; background: #0f2633; }.chain-flow small { color: #7f9aa7; font-size: 12px; }.chain-flow strong { color: #24d4c4; font: 600 21px var(--font-numeric); margin: 3px 0; }.chain-flow span { color: #587583; font-size: 11px; }.chain-flow > i { width: 22px; height: 1px; background: #3b7181; position: relative; }.chain-flow > i::after { content: ''; position: absolute; right: 0; top: -3px; border-left: 5px solid #3b7181; border-top: 3px solid transparent; border-bottom: 3px solid transparent; }
.chain-metrics { height: 52px; margin: 0 10px; border-top: 1px solid #193647; display: flex; align-items: center; justify-content: space-around; color: #718c99; font-size: 11px; }.chain-metrics b { color: #d8e8ec; margin-left: 4px; font: 600 12px var(--font-numeric); }
.suggestion-list { padding: 5px 8px; height: calc(100% - 32px); overflow: auto; }.suggestion-row { min-height: 49px; display: grid; grid-template-columns: 25px 1fr 105px; align-items: center; gap: 7px; border-bottom: 1px solid #17303f; }.suggestion-row:last-child { border-bottom: 0; }.suggestion-row > b { color: #24d4c4; font: 600 15px var(--font-numeric); }.suggestion-row view { min-width: 0; }.suggestion-row strong, .suggestion-row small { display: block; overflow: hidden; white-space: nowrap; text-overflow: ellipsis; }.suggestion-row strong { color: #d5e5e9; font-size: 11px; }.suggestion-row small { color: #6a8593; font-size: 12px; margin-top: 3px; }.suggestion-row > span { color: #f3b33d; font-size: 11px; line-height: 1.35; text-align: right; }
.empty-state { height: 100%; display: grid; place-items: center; color: #67818e; font-size: 11px; }.error-toast { position: fixed; left: 50%; top: 72px; transform: translateX(-50%); z-index: 30; padding: 9px 14px; color: #fff; background: #9b3c45; font-size: 11px; }.page-state,.empty-business { position: fixed; inset: 0; z-index: 28; display: grid; place-items: center; color: #9fbbc5; background: rgba(7,17,28,.88); font-size: 13px; }.page-state-error { color: #ff9b9b; }.empty-business { inset: 166px 12px 10px; z-index: 5; background: rgba(7,17,28,.72); pointer-events: none; }
...
.module-heading { height: 54px; padding: 0 16px; display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid #193647; background: #0d202d; }.module-heading small { display: block; color: #527582; font-size: 12px; }.module-heading h2 { margin: 3px 0 0; color: #dcebed; font-size: 14px; letter-spacing: 0; }.module-heading > span { color: #7795a2; font-size: 11px; }
...
.region-table { min-height: 38px; display: grid; grid-template-columns: 1.5fr .6fr 1fr .6fr .7fr .7fr; align-items: center; gap: 8px; padding: 0 14px; border: 0; border-bottom: 1px solid #17303f; border-radius: 0; color: #9cb2bc; background: transparent; font-size: 11px; text-align: left; }.region-table.table-head { min-height: 34px; color: #5f7d8a; background: #091722; }.region-table-scroll { height: calc(100% - 88px); overflow: auto; }.region-table-scroll button { width: 100%; margin: 0; cursor: pointer; }.region-table-scroll button:hover { background: #102836; }.region-table strong { color: #d1e2e6; }.region-table b { color: #6fce83; text-align: right; }.region-table b.negative { color: #f26464; }
.regulatory-list-panel { grid-column: span 8; grid-row: span 2; }.regulatory-metrics-panel, .settlement-panel { grid-column: span 4; }.risk-table { width: 100%; min-height: 48px; margin: 0; display: grid; grid-template-columns: 64px minmax(0, 1fr) 150px 48px; align-items: center; gap: 10px; padding: 0 15px; border: 0; border-bottom: 1px solid #17303f; border-radius: 0; color: #8ea6b1; background: transparent; font-size: 11px; text-align: left; }.risk-table.table-head { min-height: 34px; color: #5f7d8a; background: #091722; }.risk-table-button { cursor: pointer; }.risk-table-button:hover { background: #102735; }.regulatory-scroll { height: calc(100% - 88px); overflow: auto; }.risk-table > b { width: 48px; padding: 3px 0; border: 1px solid #ad7f36; color: #f3b33d; text-align: center; font-size: 11px; }.risk-table > b.level-critical { border-color: #a9474f; color: #ff7777; }.risk-table > b.level-notice { border-color: #39738b; color: #62bde3; }.risk-table view { min-width: 0; }.risk-table strong, .risk-table small { display: block; }.risk-table strong { color: #d3e3e7; }.risk-table small { margin-top: 4px; color: #6d8996; }.risk-table code { overflow: hidden; color: #638998; text-overflow: ellipsis; white-space: nowrap; }.risk-table > span { color: #f26464; font: 600 16px var(--font-numeric); text-align: right; }
.control-grid { height: calc(100% - 54px); display: grid; grid-template-columns: repeat(2, 1fr); }.control-grid > view { min-width: 0; display: flex; flex-direction: column; justify-content: center; padding: 12px 16px; border-right: 1px solid #17303f; border-bottom: 1px solid #17303f; }.control-grid small { color: #74909d; font-size: 12px; }.control-grid strong { margin: 5px 0 3px; color: #f3b33d; font: 600 25px var(--font-numeric); }.control-grid span { color: #526f7c; font-size: 11px; }.settlement-metrics { height: calc(100% - 94px); display: grid; grid-template-columns: repeat(2, 1fr); }.settlement-metrics > view { display: flex; flex-direction: column; justify-content: center; padding: 16px; border-right: 1px solid #17303f; }.settlement-metrics small { color: #74909d; font-size: 12px; }.settlement-metrics strong { margin-top: 8px; color: #24d4c4; font: 600 22px var(--font-numeric); }.settlement-note { height: 40px; padding: 0 16px; border-top: 1px solid #17303f; color: #6e8996; font-size: 12px; line-height: 40px; }
.empowerment-gap-panel { grid-column: span 4; grid-row: span 2; }.empowerment-list-panel, .empowerment-chain-panel { grid-column: span 8; }.empowerment-scroll { height: calc(100% - 54px); overflow: auto; }.empowerment-row { min-height: 70px; display: grid; grid-template-columns: 38px minmax(0, 1fr) 180px; align-items: center; gap: 12px; padding: 8px 16px; border-bottom: 1px solid #17303f; }.empowerment-row > b { color: #24d4c4; font: 600 18px var(--font-numeric); }.empowerment-row view { min-width: 0; }.empowerment-row small { color: #527582; font-size: 12px; }.empowerment-row strong { display: block; margin-top: 3px; color: #d3e4e8; font-size: 12px; }.empowerment-row p { margin: 4px 0 0; overflow: hidden; color: #7894a0; font-size: 11px; text-overflow: ellipsis; white-space: nowrap; }.empowerment-row > span { color: #f3b33d; font-size: 11px; line-height: 1.5; text-align: right; }.empowerment-chain { height: calc(100% - 104px); display: flex; align-items: center; justify-content: center; }.empowerment-chain > view { width: 126px; height: 82px; display: flex; flex-direction: column; align-items: center; justify-content: center; border: 1px solid #2b596b; background: #0f2633; }.empowerment-chain small { color: #7894a0; font-size: 12px; }.empowerment-chain strong { margin-top: 7px; color: #24d4c4; font: 600 25px var(--font-numeric); }.empowerment-chain > i { width: 42px; height: 1px; background: #3b7181; }.opportunity-strip { height: 50px; display: flex; align-items: center; justify-content: space-around; border-top: 1px solid #193647; color: #718c99; font-size: 11px; }.opportunity-strip b { margin-left: 6px; color: #d8e8ec; font: 600 13px var(--font-numeric); }
.modal-mask { position: fixed; inset: 0; z-index: 40; display: grid; place-items: center; background: rgba(2, 8, 13, .78); }.definition-dialog { width: min(620px, 80vw); max-height: 76vh; overflow: auto; border: 1px solid #356579; background: #0b1b28; box-shadow: 0 18px 70px rgba(0,0,0,.45); }.definition-dialog header { height: 64px; padding: 0 18px; display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid #234353; }.definition-dialog h2 { margin: 0; font-size: 17px; }.definition-dialog p { margin: 5px 0 0; color: #7893a2; font-size: 11px; }.definition-dialog button { width: 30px; height: 30px; margin: 0; padding: 0; border-radius: 0; border: 1px solid #325467; color: #bdd0d7; background: transparent; line-height: 28px; cursor: pointer; }.definition-dialog dl { margin: 0; padding: 8px 18px 18px; }.definition-dialog dt { margin-top: 12px; color: #24d4c4; font-size: 12px; }.definition-dialog dd { margin: 5px 0 0; color: #9fb5bf; font-size: 11px; line-height: 1.7; }
.drawer-mask { position: fixed; inset: 0; z-index: 42; background: rgba(2,8,13,.72); }.risk-drawer { position: absolute; top: 0; right: 0; width: min(520px, 90vw); height: 100%; outline: 0; border-left: 1px solid #356579; background: #0b1b28; box-shadow: -18px 0 70px rgba(0,0,0,.45); }.risk-drawer > header { min-height: 116px; padding: 22px 24px; display: flex; justify-content: space-between; gap: 18px; border-bottom: 1px solid #234353; }.risk-drawer header small { color: #5f8291; font-size: 12px; }.risk-drawer h2 { margin: 6px 0; font-size: 20px; }.risk-drawer header p { margin: 0; color: #8ba3ad; font-size: 11px; line-height: 1.6; }.risk-drawer header button { width: 32px; height: 32px; flex: 0 0 32px; border: 1px solid #325467; border-radius: 0; color: #bdd0d7; background: transparent; cursor: pointer; }.risk-drawer-summary { height: 74px; display: flex; align-items: baseline; gap: 10px; padding: 18px 24px; border-bottom: 1px solid #193647; }.risk-drawer-summary span { padding: 3px 7px; border: 1px solid #ad7f36; color: #f3b33d; font-size: 11px; }.risk-drawer-summary span.level-critical { border-color: #a9474f; color: #ff7777; }.risk-drawer-summary strong { color: #f26464; font: 600 28px var(--font-numeric); }.risk-drawer-summary small { color: #718d99; }.risk-subject-list { height: calc(100% - 190px); overflow: auto; padding: 8px 24px 24px; }.risk-subject-list article { padding: 15px 0; border-bottom: 1px solid #193647; }.risk-subject-list article > view { display: flex; justify-content: space-between; gap: 12px; }.risk-subject-list strong { color: #d8e8ec; font-size: 12px; }.risk-subject-list span,.risk-subject-list small { color: #5f7f8d; font-size: 12px; }.risk-subject-list p { margin: 8px 0; color: #8fa7b1; font-size: 11px; line-height: 1.6; }
...
.toolbar { min-width: 0; }.toolbar select { max-width: 104px; font-size: 13px; }.toolbar select:disabled { color: #718b97; }
...
@media (max-height: 700px) {
  .dashboard-shell { min-height: 0; padding: 7px 9px; grid-template-rows: 50px 88px minmax(0, 1fr) 168px; gap: 6px; }
  .kpi { padding: 7px 10px 6px; }.kpi-value strong { font-size: 24px; }
  .panel-body,.list-body { min-height: 0; overflow: auto; }
  .side-column { overflow-y: auto; grid-template-rows: repeat(3, 100px); }
}
@media (max-width: 1450px) {
